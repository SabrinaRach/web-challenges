...
// Try to find an object with the corresponding lastName and return the found object 
// (instead of 'null')
// Check the browser preview and conduct some tests to ensure it works correctly.

// function has two parameters: employees (array of employee objects) and lastName (string). It returns the first employee object that matches the lastName or null if no match is found.
function findByLastName(employees, lastName) {
  return employees.find((employee) => employee.lastName === lastName) || null;

  // Exercise 2:
  // The same as above, but now you are searching by the 'id'.
}
function findById(employees, id) {
  // function has two parameters: employees (array of employee objects) and id (string). It returns the first employee object that matches the id or null if no match is found.
...
function findByCitySubString(employees, string) {
  return employees.find((employee) => employee.city.includes(string)) || null;
}
...
function findByProfessionSubStringAndAge(employees, string, age) {
  return employees.find((employee) => employee.profession.includes(string) && employee.age > age) ||   null;
}
